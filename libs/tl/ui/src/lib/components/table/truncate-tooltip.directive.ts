...
export class TlTruncateTooltipDirective {
  ...
  private createTooltip(text: string) {
    ...
    // Thêm các class style CSS Enterprise cho đẹp lung linh
    this.renderer.setStyle(this.tooltipEl, 'position', 'fixed');
    this.renderer.setStyle(this.tooltipEl, 'background', '#384458'); // Màu xám tối sang trọng
    ...
    // Đưa vào body để không bị cha che khuất (overflow)
    this.renderer.appendChild(document.body, this.tooltipEl);

    // Tính toán tọa độ hiển thị ngay trên đầu vị trí chuột/ô dữ liệu
